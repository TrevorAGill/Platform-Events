trigger Launch_Async_Class on Launch_Async_Class__e (after insert) {
    for(Launch_Async_Class__e event : trigger.new){
        if(String.isNotBlank(event.Class_Name__c)){
            ...
            if(classInstance instanceof Database.Batchable<sObject>) { // If the instantiated class is a batchable class, execute a batch
